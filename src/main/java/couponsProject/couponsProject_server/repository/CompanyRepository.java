...
/**
 * Repository interface for CompanyDTO entity operations.
 * Extends JpaRepository for basic CRUD operations and JpaSpecificationExecutor for complex queries.
 */
public interface CompanyRepository extends JpaRepository<Company, Integer>, JpaSpecificationExecutor<Company> {

    //@Query("select c from CompanyDTO c where c.id = ?1")
    Company findCompaniesById(int companyID);

    //@Query("select c from CompanyDTO c where c.email = ?1 and c.password = ?2")
    Company findCompaniesByEmailAndPassword(String email, String password);

    //@Query("select (count(c) > 0) from CompanyDTO c where c.name = ?1 or c.email = ?2")
